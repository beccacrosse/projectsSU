...
using namespace std;
/*
 * This file initializes a constructor and deconstructor for RobinHoodHashTable
 * and implements isEmpty(), size(), contains(), insert(), and delete()
 * functions for the RobinHoodHashTable container type
 */


RobinHoodHashTable::RobinHoodHashTable(HashFunction<string> hashFn) {
    logicalSize = 0;
    maxSize = hashFn.numSlots();
    ourHash = hashFn;
    elems = new Slot[maxSize];
...
bool RobinHoodHashTable::insert(const string& elem) {
    if(logicalSize == maxSize || contains(elem)){
        return false;
    }//if theres space in there and value isnt contained
    int index = ourHash(elem);
    Slot newElem = {elem, 0};
    while (elems[index % maxSize].distance != EMPTY_SLOT){
        //index we're focusing on
        int pos = index % maxSize;
        //if distance is less than distance of og elem in the index
        if(newElem.distance > elems[pos].distance){
            //switch and repeat wth switched elem
            Slot temp = elems[pos];
            elems[pos] = newElem;
            newElem = temp;
            index ++;
            ...
            //otherwise update values to check next slot
            newElem.distance ++;
            index ++;
        }
    }
    elems[index % maxSize] = newElem;
    //update
    logicalSize ++;
    return true;
...
bool RobinHoodHashTable::contains(const string& elem) const {
    //hash index
    int hashIndex = ourHash(elem);
    //distance of our elem
    int currentDistance = 0;
    for (int i = 0; i < maxSize; i ++){
        int location = (hashIndex + currentDistance) % maxSize;
        //if it reaches an empty slot quit.
        if (elems[location].distance == EMPTY_SLOT || currentDistance > elems[location].distance)
            return false;
        //if it finds the element, return true
        else if (elem == elems[location].element){
            return true;
        //if distance is greater than distance it should be, quit
        }else
        currentDistance++;
    }
    return false;
...
bool RobinHoodHashTable::remove(const string& elem) {
    // if table doesn't contain elem
    if (!contains(elem))
        return false;
    // if table contains elem
    int hashIndex = ourHash(elem);
    // finds position of elem in table
    while (elems[hashIndex % maxSize].element != elem)
        hashIndex++;
    // next two lines remove elem from table
    elems[hashIndex % maxSize] = {"", EMPTY_SLOT};
    // elemDis is distance of element following the removed elem
    int elemDis = elems[(hashIndex + 1) % maxSize].distance;
    // while the distance of the element wants to move closer to home
    while (elemDis != 0 && elemDis != EMPTY_SLOT) {
        // // the element to the right of the previously removed elem gets shifted to the left
        elems[hashIndex % maxSize].element = elems[(hashIndex + 1) % maxSize].element;
        elems[hashIndex % maxSize].distance = elemDis - 1;
        // position of empty slot gets incremented appropriately
        hashIndex++;
        // elemDis gets distance of the value to the right of the shifted element
        elemDis = elems[(hashIndex + 1) % maxSize].distance;
        // the shifted element gets removed from table
        elems[hashIndex % maxSize].element = "";
        elems[hashIndex % maxSize].distance = EMPTY_SLOT;
    }
    // logical size reduced by 1, since the originally passed in elem got removed
    logicalSize--;
    return true;
}
 ...
 * bool RobinHoodHashTable::remove(const string& elem) {
    ...
    int index = ourHash(elem);
    //find actual value of elem
    for(int i = 0; i < maxSize; i ++){
        int location = index % maxSize;
        if(elems[location].element == elem)
            break;
        index ++;
    }
    //set slot to be empty
    Slot current = elems[index % maxSize];
    elems[index % maxSize].distance = EMPTY_SLOT;
    //update index
    while (elems[index + 1 % maxSize].distance != EMPTY_SLOT && elems[index + 1 % maxSize].distance != 0){
        //element distance goes down 1
        current = elems[index + 1 % maxSize];
        //shifts element down
        elems[index % maxSize] = elems[(index + 1) % maxSize];
        elems[index % maxSize].distance --;
        index ++;
    }
    while (elemDis != 0 && elemDis != EMPTY_SLOT) {
        // // the element to the right of the previously removed elem gets shifted to the left
...
void RobinHoodHashTable::printDebugInfo() const {
    ...
    for(int x = 0; x < logicalSize; x++){
        cout<< elems[x].element << elems[x].distance <<endl;
    }
}
