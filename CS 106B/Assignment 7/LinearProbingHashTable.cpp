...
LinearProbingHashTable::LinearProbingHashTable(HashFunction<std::string> hashFn) {
    ...
    logicalSize = 0;
    //actualSize = 0;
    //make elems initialize to empty
...
bool LinearProbingHashTable::insert(const string& elem) {
    ...
    //whle within amount of slots
    for(int i = 0; i < maxSize; i++){
        //check the location. defaults to filled. how to i get around this
        //if slot is open, or if nothing has been placed yet place value there
        if(isEmpty() || i > logicalSize ||
                elems[(hashIndex + i) % maxSize].type != SlotType::FILLED){
            //update that index
            if(elems[(hashIndex + i) % maxSize].type == SlotType::EMPTY){
                actualSize ++;
            }
            elems[(hashIndex+i) % maxSize].type = SlotType::FILLED;
            elems[(hashIndex+i) % maxSize].value = elem;
            logicalSize ++;
            return true;
        }
    }
...
bool LinearProbingHashTable::contains(const string& elem) const {
    int hashIndex = ourHash(elem);
    //checks every slot svailsble
    for(int i = 0; i < actualSize; i++){

        //if theres an empty slot, elem can't be contained in elems
        if(elems[(hashIndex + i) % maxSize].type == SlotType::EMPTY)
            break;
        //if the elem at this index is elem, it is contained
        if(elems[(hashIndex + i) % maxSize].value == elem &&
                elems[(hashIndex + i) % maxSize].type != SlotType::TOMBSTONE)
            return true;
    }
    return false;
}
...
bool LinearProbingHashTable::remove(const string& elem) {
    ...
    if(!contains(elem))
        return false;
    int hashIndex = ourHashFn(elem);
    //check hash index and spaces after
...
bool LinearProbingHashTable::insert(const string& elem) {
    // if table already contains elem or the table is full return false
    if (contains(elem) || logicalSize == maxSize)
        return false;
    int pos = ourHashFn(elem);
    // increments pos until theres an empty or tombstoned space
    while (elems[pos % maxSize].type == SlotType::FILLED) {
        pos++;
    }
    // sets the space equal to elem and marks it as filled
    elems[pos % maxSize].value = elem;
    elems[pos % maxSize].type = SlotType::FILLED;
    logicalSize++;
    return true;
}
bool LinearProbingHashTable::contains(const string& elem) const {
    int pos = ourHashFn(elem);
    // runs through every index in elems
    for (int i = 0; i < maxSize; i++) {
        // if the space where elem should be is empty, elem can't be in elems
        if (elems[pos % maxSize].type == SlotType::EMPTY)
            return false;
        // if the space where elem should be is filled and is equal to elem, elem is in elems
        else if (elems[pos % maxSize].type == SlotType::FILLED && elems[pos % maxSize].value == elem)
            return true;
        // if the space is filled and not elem or a tombstone, search next position
        else
            pos++;
    }
    return false; // if table is full and doesn't contain elem
}
...
bool LinearProbingHashTable::remove(const string& elem) {
    ...
    for (int i = 0; i < maxSize; i++) {
        ...
        // if the space where elem should be is filled and is equal to elem, elem is in elems
            ...
            // change the type at the space where elem is to TOMBSTONE and shrink logicalSize by 1
            elems[pos % maxSize].type = SlotType::TOMBSTONE;
            logicalSize--;
            return true;
        }
        // if the space is filled and not elem or a tombstone, search next position
