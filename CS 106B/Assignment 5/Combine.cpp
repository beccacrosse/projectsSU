...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    Vector<DataPoint> leftSequence = combine(sequences.subList(0, half));
    Vector<DataPoint> rightSequence = combine(sequences.subList(half));
    // now we merge
...
Vector<DataPoint> merge(const Vector<DataPoint>& left, const Vector<DataPoint>& right) {
    ...
    while (true) {
        if (left.size() == indexL) {
            merged += right.subList(indexR);
            break;
        }
        if (right.size() == indexR) {
            merged += left.subList(indexL);
            break;
        }
        pointL = left[indexL];
        pointR = right[indexR];
        if (pointL.weight < pointR.weight) {
            merged.add(pointL);
            indexL ++;
        }
        else {
