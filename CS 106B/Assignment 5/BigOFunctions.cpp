 ...
 */
void printC(int n) {
    for (int row = 0; row < n; row++) {// at this point O(n)
...
void printCycle_v1(Vector<int>& values) {
    for (int i = 0; i < values.size(); i++) { //O(n)
        int first = values[0];
        values.remove(0); // Remove first element //O/(n^2)
...
void printCycle_v2(Vector<int>& values) {
    for (int i = 0; i < values.size(); i++) {//O(n)
        int first = values[0];
        values = values.subList(1) + first; // Cycle the elements O(n) .... = O(n) * O(n)
...
int maximumSingleSellProfit_v1(const Vector<int>& values) {
    ...
    for (int i = 0; i < values.size(); i++) { //O(n)
        for (int j = i + 1; j < values.size(); j++) {//O(n)
            best = max(best, values[j] - values[i]);//O(n)
        }
    }
    return best;
}
...
int maximumSingleSellProfit_v2(const Vector<int>& values) {
    ...
    int secondHalfProfit = maximumSingleSellProfit_v2(values.subList(half)); //O(logn) * O(n) = O(nlogn)

    /* Find the best buy price in the first half and theb est buy price in the
     * second half.
     */
    int firstMin = values[0];
    for (int i = 1; i < half; i++) {//O(n/2)
        firstMin = min(firstMin, values[i]);//
    }
    int secondMax = values[half];
    for (int i = half + 1; i < values.size(); i++) {//O(n/2)
