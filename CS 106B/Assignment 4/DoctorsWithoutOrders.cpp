/*
 * This algorithm takes a map of doctors and patients respectively
 * and determines if it is possible to make a schedule that matches
 * the time availability of doctors to the time needed by each patient.
 * If a schedule is possible, a schedule is created matching each doctor to
 * the patient(s) that they will care for.
 */
...
bool canAllPatientsBeSeen(const Map<string, int>& doctors,
    ...
    //if no more patients to be seen, true
    if(patients.isEmpty()){
        return true;
    }else{
        //select first patient
        string patient = patients.firstKey();
        //if the doctor can see the patient, check if other patients can be seen
        for(string doctor: doctors){
            if( doctors[doctor] >= patients[patient]){
                //make new variables and modify them accounting new patient
                Map<string, int> remPatients = patients;
                Map<string, int> updatedDocs = doctors;
                remPatients.remove(patient);
                updatedDocs[doctor] -= patients[patient];
                schedule[doctor] += patient;

                //if every person can be seen return true;
                if(canAllPatientsBeSeen(updatedDocs, remPatients, schedule)){
                    return true;
                }
                //otherwise the schedule is returned to unchanged state
                else{
                    schedule[doctor] -= patient;
                }
            }
...
STUDENT_TEST("Creates empty schedule if no patients"){
    Map<string, Set<string>> schedule;
