/*
 * This Algorithm takes a Map of cities, each of which can
 * share supplies with the cities neighboring them, and a number
 * of cities that can be given disaster suppplies within a budget.
 * It is determined if there are certain cities (within the agreed upon
 * amount of cit) that can be given supplies and provide every city in
 * the Map can have. If this is possible, a list of the cities given supplies
 * will be created.
 */
...
bool canBeMadeDisasterReadyRec(const Map<string, Set<string>>& roadNetwork,
                            int numCities,
                            Set<string>& supplyLocations, const Set<string>& uncovered) {
    //base case: no cities
    if(roadNetwork.isEmpty()){
        ...
        Set<string> cities = roadNetwork[first] + first;
        Set<string> uncoveredCities;
        if(uncovered.contains(first)){
            //test route where supplies are placed in every neighbor
            for(string city: cities){
                supplyLocations += city;
                uncoveredCities =  uncovered - city;

                if(canBeMadeDisasterReadyRec(roadNetwork, numCities - 1, supplyLocations, uncoveredCities)){
                        return true;
                }else{
                    supplyLocations -= city;
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    if(numCities < 0){
        ...
        Set<string> uncovered = roadNetwork[city] + city;
...
bool canBeMadeDisasterReadyRec(const Map<string, Set<string>>& roadNetwork, int numCities, Set<string>& supplyLocations,
                               const Set<string>& uncoveredCities, const Set<string>& remainingCities);

bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
                            int numCities,
                            Set<string>& supplyLocations) {
    if (numCities < 0)
       error("Can't stockpile negative cities!");
    Set<string> uncoveredCities;
    for (string elem: roadNetwork) {
        uncoveredCities += elem;
    }
    Set<string> remainingCities = uncoveredCities;
    return canBeMadeDisasterReadyRec(roadNetwork, numCities, supplyLocations, uncoveredCities, remainingCities);
}


bool canBeMadeDisasterReadyRec(const Map<string, Set<string>>& roadNetwork, int numCities, Set<string>& supplyLocations,
                                Set<string> uncoveredCities, Set<string>& remainingCities) {
    if (numCities < 0)
        return false;
    if (uncoveredCities.size() == 0)
        ...
        for (string city: remainingCities) {
            Set<string> allPossibilities = roadNetwork[city];
            allPossibilities += city;
            for (string elem: allPossibilities) {
                if (uncoveredCities - roadNetwork[elem] - elem == uncoveredCities)
                    continue;
                if (!supplyLocations.contains(elem)) {
                    Set<string> uncoveredCitiesForElem = uncoveredCities;
                    uncoveredCitiesForElem -= elem;
                    uncoveredCitiesForElem -= roadNetwork[elem];
                    supplyLocations += elem;
                    if (canBeMadeDisasterReadyRec(roadNetwork, numCities - 1, supplyLocations, uncoveredCitiesForElem, remainingCities))
                        return true;
                    else {
                        uncoveredCitiesForElem += elem;
                        uncoveredCitiesForElem += roadNetwork[elem];
...
STUDENT_TEST("returns false if there is no funding for any cities"){
