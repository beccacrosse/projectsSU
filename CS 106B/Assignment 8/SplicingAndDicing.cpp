...
Nucleotide* toStrand(const string& str) {
    ...
    dna->next = nullptr;
    //first case. sets to last variable
    dna->value = str[str.length() -1];
...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    ...
    (void) target;
    return nullptr;*/
    //if empty return start of list
    Nucleotide* first = nullptr;
    if( target == nullptr){
        return dna;
    }while(dna != nullptr){
        if(dna->value == target->value){
            Nucleotide* temp = target->next;
            first = dna;
            while(temp != nullptr){
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    ...
    return false;*/
        ...
        // if it's not possible to splice return false
         if (findFirst(dna, target) == nullptr) return false;
         dna = findFirst(dna, target);
         Nucleotide* temp;
         // handles case if splice is at beginning
         if (dna->prev == nullptr) {
             while (target->next != nullptr) {
                 temp = dna->next;
                 delete dna;
                 dna = temp;
                 // since dna is the beginning element
                 dna->prev = nullptr;
                 target = target->next;
             }
             // handles cases where target's linked list is same length as dna's linked list
             if (dna->next == nullptr) {
                 // deletes the linked list
                 delete dna;
                 dna = nullptr;
             }
             // handles cases where target's linked list was shorter than dna's linked list
             else {
                 temp = dna->next;
                 delete dna;
                 // sets dna to beginning of its linked list
                 dna = temp;
                 dna->prev =  nullptr;
             }
         }
    //handles cases where splice is not at beginning
         else {
             // deletes every Nucleotide in dna that needs to be deleted unless
             // the splice reaches the end of dna's linked list
             while (target != nullptr && dna->next != nullptr) {
                 // to link linked list after deleting dna
                 temp = dna->prev;
                 // to preserve values after deleting dna
                 char letter = dna->next->value;
                 // links past dna to prep for deleting dna
                 temp->next = dna->next;
                 // deletes memory for dna
                 delete dna;
                 // dna is now what dna->next used to be
                 dna = temp->next;
                 // since dna is now linked to temp
                 dna->prev = temp;
                 // since dna's value was dna->next's value
                 dna->value = letter;
                 // go to next Nucleotide needing to be deleted
                 target = target->next;
             }
             // handles case where splice reaches end of dna's linked list
             if (dna->next == nullptr) {
                 temp = dna->prev;
                 temp->next = dna->next;
                 delete dna;
             ...
             // rewinds dna to beginning of its linked list
             while (dna->prev != nullptr) {
                 dna = dna->prev;
             }
         }
         // generally returns true if findFirst(dna, target) returns true
         return true;
}
...
/*
STUDENT_TEST("handles uninitialized pointers") {
    Nucleotide* dna = new Nucleotide;
...
STUDENT_TEST("deleteNucleotides handles nucleotide with one element") {
    Nucleotide* dna = new Nucleotide;
    dna->value = 'A';
    dna->next = nullptr;
    dna->prev = nullptr;
...
STUDENT_TEST("fromDNA works on strings not containing 'A', 'C', 'G', or 'T'"){
...
PROVIDED_TEST("deleteNucleotides works on not containing 'A', 'C', 'G', or 'T'") {
    Nucleotide* dna = new Nucleotide;
    dna->value = '!';
    dna->prev = nullptr;
