...
PROVIDED_TEST("Reports errors on invalid inputs.") {
    Vector<int> result;
    EXPECT_ERROR(hasSquareSequence(-137, result));
}
...
PROVIDED_TEST("Stress test: Can order the sequence 1, 2, ..., 35 (should take at most two seconds)") {
    Timing::Timer timer;

    Vector<int> result;
    timer.start();
    bool exists = hasSquareSequence(35, result);
    timer.stop();

    checkCorrectness(35, exists, result);
    EXPECT_LESS_THAN(timer.timeInSeconds(), 2); // Needs to be fast enough
}
